...
router.post("/", function (req, res, next) {
  ...
  let song_artist = song_artistOri
    .replace("/,/g", "&")
    .replace("/，/g", "&")
    .replace(",");
  ...
  database.query(sql, sqlObj, (err, data) => {
    ...
    if (data.length >= 1) {
      ...
      let sql2 =
        "update listeninfo set presstimes=? where id=? and song_artist=?";
      ...
      //id和song_artist对没有找到，需要添加

      let pressTimeList = [4.0, 4.3, 4.6, 5.0, 5.5, 6.0, 6.5, 7.0];
      let pressTime =
        pressTimeList[Math.floor(Math.random() * pressTimeList.length)];
      console.log("now insert");
      let sql3 =
        "insert into listeninfo set id=?,song_artist=?,presstimes=?,songUrl=?,platform=?,songid=?,albumid=?";
      let sqlArr3 = [
        id,
        song_artist,
        pressTime,
      ...
      console.log("listen info", sqlArr3);
